...
Thanks to @jipis for fixing internet pricing and identifying how to find specials for individual passengers. I changed the logic to check every single passenger's order to find specials only available to them (like an unlisted 40% refreshment package only available to teans) which the code was not finding. Code also avoids checking orders multiple times, which can happen when reservations are linked. For linked reservations, the passenger may appear to be in the wrong room. This is just a feature of the code which I will try to fix in the future, the passengers are booked in the correct room!
